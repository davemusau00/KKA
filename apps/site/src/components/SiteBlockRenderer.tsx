...
export function SiteBlockRenderer({block,data}:{block:SiteBlock;data:SiteBootstrap}){
 ...
 switch(block.blockType){
  ...
  case 'PRACTICE_GRID': {const requested=strings(c.slugs);const items=requested.length?data.practiceAreas.filter(a=>requested.includes(a.slug)):data.practiceAreas;return <Section tone={sectionTone}><Container><Eyebrow>{text(c.eyebrow,'Our Services')}</Eyebrow><Heading>{text(c.title,'AREAS OF PRACTICE')}</Heading>{c.description&&<p className="cms-section-copy">{text(c.description)}</p>}<div className="practice-index-grid">{items.map(a=><PracticeCard key={a.id} area={a}/>)}</div></Container></Section>}
  ...
  case 'METRICS': return <section className={`metrics cms-metrics theme-${sectionTone}`}><Container className="metrics-grid">{data.metrics.map(m=><div key={m.id}><strong>{m.value}</strong><span>{m.label}</span></div>)}</Container></section>;
