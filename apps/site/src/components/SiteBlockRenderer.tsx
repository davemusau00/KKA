...
export function SiteBlockRenderer({block,data}:{block:SiteBlock;data:SiteBootstrap}){
 ...
 switch(block.blockType){
  ...
  case 'INSIGHTS_GRID': {const requested=strings(c.slugs);const items=requested.length?data.publications.filter(p=>requested.includes(p.slug)):data.publications;return <Section tone={sectionTone}><Container><Eyebrow>{text(c.eyebrow)}</Eyebrow><Heading>{text(c.title)}</Heading><PublicationCollection items={items} pageSize={Number(c.limit)||12}/></Container></Section>}
