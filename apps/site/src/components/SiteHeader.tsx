...
export function SiteHeader({settings}:{settings:SiteSettings}){
 ...
 useEffect(()=>{
  ...
  const resize=()=>{if(innerWidth>=1100)setOpen(false);};addEventListener('keydown',key);addEventListener('resize',resize);
 ...
 return <header className={`site-header ${compact?'is-compact':''}`}><div className="site-header-inner"><Link className="brand" to="/" aria-label={`${settings.firmName} home`}>{brand}</Link><nav aria-label="Primary navigation">{nav.map(([label,to])=><a key={to} href={to}>{label}</a>)}</nav><div className="header-actions"><Link className="search-button" to="/search" aria-label="Search"><Search size={17}/></Link><Link className="header-cta" to="/contact">Contact Us ?</Link><button ref={toggle} className="menu-button" onClick={()=>setOpen(true)} aria-label="Open menu" aria-controls="public-menu" aria-expanded={open}><Menu/></button></div></div>
 <div ref={sheet} id="public-menu" className={`mobile-menu ${open?'open':''}`} role="dialog" aria-modal={open||undefined} aria-label="Site navigation" aria-hidden={!open} inert={!open}><div className="mobile-menu-head"><div className="brand">{brand}</div><button onClick={()=>setOpen(false)} aria-label="Close menu"><X/></button></div><nav>{nav.map(([label,to],i)=><a key={to} href={to} onClick={()=>setOpen(false)} style={{transitionDelay:`${i*30}ms`}}>{label}</a>)}</nav><div className="mobile-contact"><span>Talk to the firm</span><a href={`tel:${settings.phone.replace(/[^+\d]/g,'')}`}>{settings.phone}</a><a href={`mailto:${settings.email}`}>{settings.email}</a></div></div></header>;
}
