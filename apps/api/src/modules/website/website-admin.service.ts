...
export class WebsiteAdminService {
  ...
  async savePage(user:RequestUser,raw:unknown){
    ...
    const result=await this.prisma.client.$transaction(async tx=>{
      ...
      await tx.websitePageVersion.create({data:{pageId:page.id,version,title:page.title,description:page.description,seo:page.seo,snapshot:{heroAssetId:page.heroAssetId,scheduledFor:page.scheduledFor,blocks:input.blocks},createdById:user.id}});
