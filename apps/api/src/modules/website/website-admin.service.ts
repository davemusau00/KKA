...
export class WebsiteAdminService {
  ...
  async saveSettings(user:RequestUser,raw:unknown) {
    const input=SiteSettingsSchema.parse(raw);await this.validateEditorial(user,input);
  ...
  async savePage(user:RequestUser,raw:unknown){
    const input=SavePageSchema.parse(raw);await this.validateEditorial(user,input);const slug=slugify(input.slug||input.title);
    const result=await this.prisma.client.$transaction(async tx=>{
      ...
      if(existing && input.currentVersion!==existing.currentVersion)throw new ConflictException('This page changed in another session. Reload before saving.');
      if(existing){
        const claimed=await tx.websitePage.updateMany({where:{id:existing.id,firmId:user.firmId,currentVersion:input.currentVersion},data:{currentVersion:{increment:1}}});
        if(claimed.count!==1)throw new ConflictException('This page changed in another session. Reload before saving.');
      }
  ...
  async restorePageVersion(user:RequestUser,id:string,version:number){
    ...
    const snapshot=v.snapshot as any;return this.savePage(user,{id:current.id,currentVersion:current.currentVersion,slug:current.slug,title:v.title,description:v.description,status:'DRAFT',seo:v.seo,heroAssetId:snapshot?.heroAssetId||null,blocks:snapshot?.blocks||[]});
  }
  async deletePage(user:RequestUser,id:string){const p=await this.page(user,id);await this.prisma.client.websitePage.update({where:{id:p.id},data:{status:'ARCHIVED'}});await this.audit.record({firmId:user.firmId,actorUserId:user.id,action:'website.page_deleted',entityType:'website_page',entityId:id,metadata:{slug:p.slug}});return{ok:true};}
  ...
  async saveTestimonial(user:RequestUser,raw:unknown){const input=SaveTestimonialSchema.parse(raw);await this.validateEditorial(user,input);const {id,...data}=input;const r=id?await this.updateScoped('websiteTestimonial',user,id,data):await this.prisma.client.websiteTestimonial.create({data:{firmId:user.firmId,...data}});await this.auditCms(user,'testimonial_saved',r.id);return r;}
  ...
  async saveForm(user:RequestUser,raw:unknown){const input=SaveFormSchema.parse(raw);await this.validateEditorial(user,input);const{id,...rest}=input;const previous=id?await this.ensureScope('websiteFormDefinition',user,id):null;const data={...rest,key:slugify(rest.key),version:previous?previous.version+1:1,schema:rest.schema as Prisma.InputJsonValue,routing:rest.routing as Prisma.InputJsonValue};const r=id?await this.updateScoped('websiteFormDefinition',user,id,data):await this.prisma.client.websiteFormDefinition.create({data:{firmId:user.firmId,...data}});await this.auditCms(user,'form_saved',r.id,{key:r.key});return r;}
  ...
  private async validateEditorial(user:RequestUser,input:any){
    if(['APPROVED','SCHEDULED'].includes(input.status)&&!user.permissions.includes('website.review'))throw new ForbiddenException('Review permission is required to approve content.');
    if(input.status==='PUBLISHED')input.status='DRAFT';
    if(input.status==='SCHEDULED'&&!input.scheduledFor)throw new BadRequestException('Choose a publication time.');
    const visit=async(value:any,key=''):Promise<void>=>{
      if(value===null||value===undefined)return;
      if(typeof value==='string'){
        if(/(?:url|href|src)$/i.test(key)&&value&&!/^(?:\/(?!\/)|#|https?:\/\/|mailto:|tel:)/i.test(value))throw new BadRequestException('Links must use an approved URL scheme.');
        if(['heroAssetId','imageId','coverId','assetId','videoAssetId'].includes(key)&&value&&!await this.prisma.client.websiteMediaAsset.findFirst({where:{id:value,firmId:user.firmId}}))throw new BadRequestException('Media must belong to this firm.');
        if(key==='authorProfileId'&&value&&!await this.prisma.client.websiteProfessionalProfile.findFirst({where:{id:value,firmId:user.firmId}}))throw new BadRequestException('Author must belong to this firm.');
        if(key==='userId'&&value&&!await this.prisma.client.user.findFirst({where:{id:value,firmId:user.firmId}}))throw new BadRequestException('User must belong to this firm.');
      }
      if(typeof value==='object')for(const [k,v]of Object.entries(value)){
        if(['customCss','customJavaScript','fontFamily','style','script'].includes(k))throw new BadRequestException('Public design properties are governed by the design system.');
        await visit(v,k);
      }
    };await visit(input);
  }
