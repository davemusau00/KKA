...
// ==========================================
// 2B. CONFIGURATION STUDIO & ADMIN TYPES
// ==========================================
export type FirmMarkType =
  | 'firm_seal'
  | 'branch_seal'
  | 'logo'
  | 'received_stamp'
...
export interface FirmMarkAsset {
  id: string;
  name: string;
  markType: FirmMarkType;
  branchId?: string;
  description: string;
  textBadge: string;
  authorizedRoleIds: RoleId[];
  colorScheme: 'gold' | 'emerald' | 'rose' | 'blue' | 'slate' | 'amber';
  isDefault: boolean;
  createdAt: string;
}
...
export interface SignatureProfile {
  ...
  signatureType: 'scanned_image' | 'drawn' | 'typed_legal';
  executionBlockText: string;
...
export interface CustomFieldDefinition {
  ...
  key: string;
  label: string;
  fieldType: 'text' | 'number' | 'currency' | 'date' | 'select' | 'boolean';
  options?: string[];
  required: boolean;
  stageApplicability?: number;
  description?: string;
}
...
export interface NumberingSchemeConfig {
  matterPrefix: string;
  matterFormat: string;
  matterSeqPadding: number;
  clientPrefix: string;
  feeNotePrefix: string;
  receiptPrefix: string;
  paymentVoucherPrefix: string;
  sequenceResetPeriod: 'yearly' | 'monthly' | 'never';
...
export interface BackupSnapshot {
  ...
  sizeBytes: number;
  createdAt: string;
  type: 'manual' | 'scheduled_daily' | 'pre_upgrade';
...
export interface SystemHealthMetrics {
  vpsUptimeSeconds: number;
  nodeVersion: string;
  dbStatus: 'healthy' | 'degraded' | 'disconnected';
  dbConnections: number;
  redisStatus: 'connected' | 'idle';
