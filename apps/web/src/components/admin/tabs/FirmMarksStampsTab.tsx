  ...
  return <div className="admin-tab-content space-y-5">
    ...
    {section === 'marks' ? <>
      <div className="grid gap-3 md:grid-cols-2">{marks.map(m=><article key={m.id} className="admin-card space-y-2"><h3 className="font-semibold">{m.displayName}</h3><p>{words(m.type)} · {m.active ? 'Available' : 'Retired'}</p>
        {m.versions.length ? <div className="flex flex-wrap gap-3">{m.versions.map(v=><figure key={v.id}><img className="w-20 h-20 object-contain bg-white rounded p-1" src={`/api/v1/marks/versions/${v.id}/preview`} alt={`${m.displayName} version ${v.version}`} /><figcaption className="text-xs">Version {v.version}</figcaption></figure>)}</div> : <p>No uploaded artwork.</p>}
      ...
      <div className="grid gap-3 md:grid-cols-2">{signatures.map(s=><article className="admin-card space-y-2" key={s.id}><h3>{s.professionalDisplayName}</h3><p>{s.approvalStatus}</p><div className="flex gap-2 flex-wrap">{s.versions.map(v=><figure key={v.id}><img className="w-32 h-16 bg-white object-contain" src={`/api/v1/marks/signature-versions/${v.id}/preview`} alt={`Signature version ${v.version}`} /><figcaption>v{v.version} {v.active?'Current':'Previous'}</figcaption></figure>)}</div>{canManage && <button className="admin-btn-secondary" disabled={busy || !s.versions.length} onClick={()=>run(()=>apiClient.post('/marks/signature-profiles',{userId:s.userId,professionalDisplayName:s.professionalDisplayName,typedSignatureAllowed:s.typedSignatureAllowed,approvalStatus:s.approvalStatus==='APPROVED'?'RETIRED':'APPROVED'}))}>{s.approvalStatus==='APPROVED'?'Retire profile':'Approve current version'}</button>}</article>)}</div>
