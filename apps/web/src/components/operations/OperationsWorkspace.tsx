...
export const OperationsWorkspace: React.FC = () => {
  ...
  const [projectWork, setProjectWork] = useState({ milestoneTitle: '', milestoneDueAt: '', spendDescription: '', spendAmount: '', spendOccurredAt: '' });
            ...
            <div className="space-y-4">
              ...
              {selectedProject && <>
                ...
                <div className="grid gap-3 lg:grid-cols-2">
                  ...
                  <form className="rounded-xl border border-slate-800 p-3" onSubmit={(event) => { event.preventDefault(); void run(async () => { const amount = Number(projectWork.spendAmount); if (!projectWork.spendDescription.trim() || !Number.isFinite(amount) || amount <= 0 || !projectWork.spendOccurredAt) throw new Error('Description, a positive amount, and the date are required.'); await operationsApi.recordProjectSpend(selectedProject.id, { description: projectWork.spendDescription.trim(), amount, occurredAt: toIso(projectWork.spendOccurredAt)! }); setProjectWork((current) => ({ ...current, spendDescription: '', spendAmount: '', spendOccurredAt: '' })); }, 'Manual project spend recorded.'); }}>
                ...
                <div className="grid gap-3 lg:grid-cols-2">
                  ...
                  <div className="rounded-xl border border-slate-800 p-3"><div className="mb-2 text-xs font-semibold text-slate-200">Manual spend register</div><div className="space-y-2">{selectedProject.spend?.map((spend) => <div key={spend.id} className="rounded-lg bg-slate-950/60 p-2 text-xs"><div className="flex items-center justify-between gap-2"><span className="font-medium text-slate-200">{spend.description}</span><span className="text-amber-300">{money(spend.amount)}</span></div><div className="mt-1 text-slate-500">{dateTime(spend.occurredAt)} · {spend.source}</div></div>)}{!selectedProject.spend?.length && <p className="py-3 text-xs text-slate-500">No manual spend recorded.</p>}</div></div>
                </div>
              </>}
              <div className="grid gap-3 lg:grid-cols-2">
