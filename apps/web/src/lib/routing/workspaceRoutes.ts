export type WorkspaceRoute = {
  workspace: string;
  resourceType?: "client" | "task" | "document" | "proceeding" | "court_event" | "filing" | "service_record" | "project" | "meeting" | "knowledge_item";
  resourceId?: string;
  matterId?: string;
  matterTab?: string;
};
...
export function parseWorkspaceLocation(pathname: string, search = ""): WorkspaceRoute {
  ...
  const resourceMatch = normalized.match(/^\/(clients|tasks|documents|knowledge)\/([^/]+)$/)
