...
- Password-recovery browser journey, MFA, administrative session inspection/revocation, and first-login onboarding remain incomplete. Invite acceptance has an API and direct browser route, but browser automation, cross-user verification, and mail delivery remain open. The password-reset API and self-service logout-all slice are locally accepted.
